...
// ====== 动态加载 crypto-js（登录后才需要） ======
function loadCryptoJS() {
    if (cryptoLoaded || typeof CryptoJS !== 'undefined') { cryptoLoaded = true; return Promise.resolve(); }
    return new Promise((resolve, reject) => {
        const s = document.createElement('script');
        s.src = 'https://cdnjs.cloudflare.com/ajax/libs/crypto-js/4.2.0/crypto-js.min.js';
        s.onload = () => { cryptoLoaded = true; resolve(); };
        s.onerror = reject;
...
function renderCurrentPage() {
    ...
    else if (currentPage === 'timers') renderTimers();
    else if (currentPage === 'goals') { renderGoals(); renderCountdowns(); }
    else if (currentPage === 'diary') { renderDiaries(); renderVaultState(); }
...
function renderTimers() {
    ...
    list.innerHTML = items.map(t => {
        ...
        return `<div class="timer-item" style="border-left:3px solid ${t.color}" data-id="${t.id}" data-date="${t.date}" data-direction="${t.direction}">
            <span class="timer-del" onclick="delTimer('${t.id}')">✕</span>
            <span class="timer-tag ${t.direction}">${t.direction==='countdown'?'倒计时':'正向'}</span>
            <h3>${t.title}</h3>
            <div class="timer-display ${expired?'timer-expired':''}">
                <span class="td-d">${d}</span><span class="unit">天</span>
                <span class="td-h">${h}</span><span class="unit">时</span>
                <span class="td-m">${m}</span><span class="unit">分</span>
                <span class="td-s">${s}</span><span class="unit">秒</span>
            </div>
...
function tickTimers() {
    if (currentPage !== 'timers') return;
    const items = document.querySelectorAll('.timer-item');
    const now = Date.now();
    items.forEach(item => {
        const target = new Date(item.dataset.date).getTime();
        const direction = item.dataset.direction;
        let diff = direction === 'countdown' ? target - now : now - target;
        const expired = diff <= 0;
        if (expired && direction === 'countdown') diff = 0;
        ...
        if (display) {
            display.classList.toggle('timer-expired', expired && direction === 'countdown');
            const dEl = display.querySelector('.td-d'); if (dEl) dEl.textContent = d;
            const hEl = display.querySelector('.td-h'); if (hEl) hEl.textContent = h;
            const mEl = display.querySelector('.td-m'); if (mEl) mEl.textContent = m;
            const sEl = display.querySelector('.td-s'); if (sEl) sEl.textContent = s;
        }
    });
}
...
function renderVaultList() {
    ...
    list.innerHTML = items.map(v => {
        let title = v.title;
        try { title = vaultKey && typeof CryptoJS !== 'undefined' ? CryptoJS.AES.decrypt(v.title, vaultKey).toString(CryptoJS.enc.Utf8) : '🔒 加密内容'; } catch(e) { title = '🔒 加密内容'; }
...
setInterval(() => { if (token) { loadData().then(renderAll); } }, 10000);        document.getElementById('auth-submit').textContent = authMode === 'login' ? '登录' : '注册';
        document.getElementById('auth-error').textContent = '';
        document.getElementById('auth-password-confirm').classList.toggle('hidden', authMode !== 'register');
        document.getElementById('security-fields').classList.toggle('hidden', authMode !== 'register');
        document.getElementById('password-rules').classList.toggle('hidden', authMode !== 'register');
