...
let currentFilter='all',currentTimerFilter='all',currentStatsRange='day',currentVaultFilter='all';
let editingDiaryId=null,editingVaultId=null,vaultUnlocked=false,vaultKey='',isAdmin=false;
...
function relTime(d){
  ...
  if(day===0)return'今天';if(day===1)return'昨天';if(day===2)return'前天';
  if(day<30)return day+'天前';return fmtDate(d);
}
...
function logout(){token='';localStorage.removeItem('token');showAuth();}
...
document.getElementById('auth-submit').addEventListener('click',async()=>{
  ...
  try{
    const res=await api('/api/'+authMode,'POST',{username,password});
...
async function loadData(){try{data=await api('/api/data');}catch(e){}}
async function saveData(){try{await api('/api/data','POST',data);}catch(e){}}
...
function renderTodos(){
  ...
  list.innerHTML=items.length===0?'<div class="empty-tip">暂无待办事项</div>':items.map(t=>
    ...
    '<span class="todo-del" onclick="delTodo(\''+t.id+'\')">✕</span></div>'
  ).join('');
...
function renderStats(){
  ...
  (Object.keys(cats).length?Object.entries(cats).map(([name,count])=>
    '<div class="stat-row"><span class="name">'+name+'</span><div class="bar"><div class="bar-fill" style="width:'+count/todosInRange.length*100+'%;background:'+(catColors[name]||'#888')+'"></div></div><span class="val">'+count+'</span></div>'
...
function renderDiaries(){
  ...
  list.innerHTML=sorted.map(d=>
    ...
    '<div class="diary-preview">'+(d.content||'').slice(0,80)+'</div></div>'
  ).join('');
}
function addDiary(){
...
function renderVaultList(){
  ...
  list.innerHTML=items.map(v=>{
    let title='🔒 加密内容';
    try{if(vaultKey)title=CryptoJS.AES.decrypt(v.title,vaultKey).toString(CryptoJS.enc.Utf8);}catch(e){}
...
async function renderAdmin(){
  try{
    ...
    document.getElementById('admin-user-list').innerHTML=stats.users.map(u=>{
      ...
      '<span class="admin-time">注册: '+fmtDate(u.createdAt)+'</span></div>';
    }).join('');
  }catch(e){}
}

function renderAll(){renderTodos();renderTimers();renderGoals();renderCountdowns();renderDiaries();}
